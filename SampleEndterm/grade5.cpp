...
private:
    ...
    bool _empty;
public:
    SchoolEnumerator(const string &fname): _empty(true) { _f = 
    new SeqInFileEnumerator<Contestant>(fname); }
...
private:
    std::string func(const School& e) const override
    {
        ...
        oss<<e.name<<", number of high jump entries: "<<e.highJumpCounter
        <<", number of students: "<<e.studentCounter<<endl;
        return oss.str();
    }
