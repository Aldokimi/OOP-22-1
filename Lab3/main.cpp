...
TEST_CASE("maxindex max removeMax","[max] [PrQueue]")
{
    ///maxindex and removeMax
    SECTION("ures sor")
    {
        PrQueue Q;
        CHECK_THROWS(Q.max()); // pass 
        CHECK_THROWS(Q.removeMax());
    ...
    SECTION("removeMaxes then add")
    {
        ...
        v.clear();
        for(int i=1; i<=3; ++i) v.push_back(Q.removeMax());
